    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-end">
      ...
      <div class="md:col-span-4 flex md:justify-end">
        <div class="flex gap-4 border-b border-primary pb-3 w-full md:w-auto">
          <span class="font-bold text-xs tracking-[0.2em] text-primary uppercase">Catalogo Selecto</span>
          <span class="text-xs tracking-[0.2em] text-on-surface-variant uppercase">/ {projects.length} Propiedades Disponibles</span>
        </div>
      </div>
    </div>
  </section>

  <!-- PROJECTS GRID — Asymmetric Swiss editorial layout -->
  <section class="py-24 md:py-32 px-6 md:px-20 max-w-[1440px] mx-auto">
    {#if projects.length === 0}
      <div class="text-center py-24 border border-outline-variant/30 bg-surface-container-low p-12">
        ...
        <h2 class="font-serif text-2xl text-on-surface mb-4">Catalogo en Actualizacion</h2>
        ...
        {#each projects as project, i}
          <div class={`group ${i % 2 !== 0 ? 'md:mt-28' : ''}`}>
            ...
            >
              ...
              />
              {#if project.heroTag}
                <span class="absolute top-6 left-6 bg-surface/90 backdrop-blur-sm text-primary font-bold px-4 py-2 text-[10px] tracking-[0.2em] uppercase border border-outline-variant/30">
                  {project.heroTag}
                </span>
              {/if}
              <div class="absolute inset-0 bg-black/25 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center backdrop-blur-[1px]">
                <span class="bg-primary text-white font-bold tracking-[0.2em] uppercase px-8 py-4 text-xs">
                  Descubrir Residencia
                </span>
              </div>
            </a>
            <div class="grid grid-cols-1 sm:grid-cols-2 border-t border-outline-variant/30 pt-6 gap-4">
              <div>
                <span class="text-[10px] font-bold tracking-[0.2em] text-primary uppercase block mb-2">{project.location}</span>
                <h2 class="font-serif text-2xl sm:text-3xl font-semibold text-on-surface group-hover:text-primary transition-colors">
                  {project.title}
                </h2>
              </div>
              <div class="flex flex-col sm:items-end justify-center">
                <span class="text-[9px] font-bold tracking-[0.2em] text-on-surface-variant uppercase mb-1">VALOR ESTIMADO</span>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-outline-variant/15">
              {#each project.specs.slice(0, 4) as spec}
                <div>
                  <span class="text-[8px] uppercase tracking-[0.15em] text-on-surface-variant block font-bold">{spec.label}</span>
